...
def read_image(filename):
	...
	if mode == 'P5':
		m = file.read()
		img = [[ord(m[i * width + j]) for j in range(width)] for i in range(height)]
	elif mode == 'P6':
		m = file.read()
		img = [[\
			[ord(m[3*(i * width + j) + 0]), ord(m[3*(i * width + j) + 1]), ord(m[3*(i * width + j) + 2])]\
...
def preprocess_image(img):
	...
	'''
	data = map(tuple, reduce(lambda x, y: x+y, img))
	# todo: task5
...
def label_image(img, cluster_centroids):
	...
	for i in range(len(img)):
		for j in range(len(img[0])):
			distances = [distance(img[i][j], centroid) for centroid in cluster_centroids]
...
def write_image(filename, img):
	...
	if isinstance(img[0][0], int):
		l = [j for i in img for j in i]
		
		file.write('P5\n')
		file.write(str(len(img[0])) + ' ' + str(len(img)) + '\n')
		file.write(str(max(l)) + '\n')
		
		file.write("".join(map(chr,l)))
	else:
		l = [k for i in img for j in i for k in j]
...
def decompress_image(cluster_labels, cluster_centroids):
	...
	[Empty RGB image as 3D list] -- img = [[[0 for _ in range(3)] for _ in range(width)] for _ in range(height)]
	'''
	if isinstance(cluster_centroids[0], int):
