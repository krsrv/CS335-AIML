...
def kmeans_iteration_one(data, centroids, distance):
	...
	distance_list = [float('inf')] * data_length
	index_list = [0] * data_length
	point_count = [0] * num_centroid
	new_centroids = [map(lambda x: 0.0, centroids[0])] * num_centroid

	for i, datum in enumerate(data):
		for j, centroid in enumerate(centroids):
			current = distance(datum, centroid)
			if current < distance_list[i]:
				distance_list[i] = current
				index_list[i] = j

	for i in range(data_length):
		point_count[index_list[i]] += 1
		new_centroids[index_list[i]] = map(lambda x, y: x + y, new_centroids[index_list[i]], data[i])

	for i in range(num_centroid):
		if point_count[i] == 0:
			new_centroids[i] = centroids[i]
		else:
			new_centroids[i] = tuple(map(lambda x: float(x) / point_count[i], new_centroids[i]))
...
def hasconverged(old_centroids, new_centroids, epsilon=1e-1):
	...
	for i in range(len(old_centroids)):
		distance = 0
		for j in range(len(old_centroids[0])):
			distance += (new_centroids[i][j] - old_centroids[i][j]) ** 2
		distance = math.sqrt(distance)
		if distance > epsilon:
			return False
...
def iteration_many(data, centroids, distance, maxiter, algorithm, epsilon=1e-1):
	...
	for i in range(1,maxiter+1):
		new_centroids = iteration_one(data, all_centroids[i-1], distance, algorithm)
		all_centroids.append(new_centroids)
		if hasconverged(all_centroids[i-1], new_centroids, epsilon):
			break
...
def performance_SSE(data, centroids, distance):
	...
	for datum in data:
		...
		for centroid in centroids:
			current = distance(centroid, datum)
			if current < distance_mean:
				distance_mean = current
		sse += distance_mean ** 2
...
def initialization_kmeansplusplus(data, distance, k):
	...
	# Add the first centroid - a random point from the dataset
	centroids.append(random.sample(data_copy,1)[0])
	data_copy.remove(centroids[0])
	
	# Compute each of the remaining k-1 cluster points
	for i in range(1,k):
		distances = []

		# Calculate the distances from non-mean points
		for datum in data_copy:
			distances.append(float('inf'))
			for j in range(len(centroids)):
				# Calculate distance to current centroid
				current = distance(centroids[j], datum)

				# Update distance if current centroid is closer
				if distances[-1] > current:
					distances[-1] = current
		...
		# This is the PMF
		distances = map(lambda x: x / total, distances)

		# Convert it to a cumulative probability function
		for i in range(1,len(distances)):
			distances[i] += distances[i-1]

		# Pick from a uniform distribution in [0,1)
		s = random.random()
		for i, d in enumerate(distances):
			# The chosen point is the point with the least index such that the
			# CDF is greater than or equal to the random number
			if d >= s:
				centroids.append(data_copy[i])
				data_copy.remove(data_copy[i])
				break
...
def kmedians_iteration_one(data, centroids, distance):
	...
	def median(linput):
		n = len(linput)
		output = list(linput[0])

		for i in range(len(output)):
			component = map(lambda x: x[i], linput)
			if n % 2 == 1:
				output[i] = sorted(component)[n // 2]
			else:
				output[i] = sum(sorted(component)[n//2-1:n//2+1])/2.0
