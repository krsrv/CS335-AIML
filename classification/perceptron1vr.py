	...
	def train( self, trainingData, trainingLabels, testData, testLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				# "*** YOUR CODE HERE ***"
				# max_score, corr_label = None, None
				# for label in self.legalLabels:
				# 	buff_score = 0
				# 	for index in range(len(trainingData[i])):
				# 		buff_score += self.weights[label][index] * trainingData[i][index]
				# 	if max_score is None:
				# 		max_score = buff_score
				# 		corr_label = label
				# 	else:
				# 		if buff_score > max_score:
				# 			max_score = buff_score
				# 			corr_label = label

				# if corr_label != trainingLabels[i]:
				# 	self.weights[corr_label] -= trainingData[i]
				# 	self.weights[trainingLabels[i]] += trainingData[i]
				data = [trainingData[i]]
				max_label = self.classify(data)[0]
