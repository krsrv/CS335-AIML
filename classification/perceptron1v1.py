	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for label in range(predicted_label):
					prediction = self.weights[frozenset([label, predicted_label])] * trainingData[i]
					if prediction <= 0:
						self.weights[frozenset([label, predicted_label])] += trainingData[i]
				for label in range(predicted_label+1, len(trainingLabels)):
					prediction = self.weights[frozenset([predicted_label, label])] * trainingData[i]
                                        print predicted_label+1
					if prediction > 0:
						self.weights[frozenset([predicted_label, label])] -= trainingData[i]
