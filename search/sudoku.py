...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        # Get the square with minimum possible values
        _, square = min((len(state[s]), s) for s in squares if len(state[s]) > 1)

        for value in state[square]:
            # For each possible value of the square, assign the value and append the 
            # resulting state to successors
            new_state = state.copy()
            if assign(new_state, square, value):
                successors.append((new_state, (square, value), 1))
