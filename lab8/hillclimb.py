...
def generate2optNeighbours(tour):
    ...
    #     for j in range(len(i+2, len(tour))):
    #         if i, j == 0, len(tour)-1:
    #             continue
    #         all_possible_neighbours.append(tour[0:i] + list(reversed(tour[i:j])) + tour[j:])
    
    all_possible_neighbours = [tour[0:i] + list(reversed(tour[i:j])) + tour[j:] for i in range(len(tour)) for j in range(i+2, len(tour))]
    all_possible_neighbours.remove(list(reversed(tour[:len(tour)-1])) + tour[len(tour)-1:])
    "*** --------------  ***"
    ...
    for i in range(len(tour)):
        for j in range(i+2, len(tour)):
            for k in range(j+2, len(tour)):
                # all_possible_neighbours.append(tour[0:i] + tour[i:j] + tour[j:k] + tour[k:])
                if i == 0 and k == len(tour)-1:
                    continue
                all_possible_neighbours.append(tour[0:i] + list(reversed(tour[j:k])) + tour[i:j] + tour[k:])
                all_possible_neighbours.append(tour[0:i] + tour[j:k] + list(reversed(tour[i:j])) + tour[k:])
                all_possible_neighbours.append(tour[0:i] + tour[j:k] + tour[i:j] + tour[k:])
                all_possible_neighbours.append(tour[0:i] + list(reversed(tour[i:j])) + list(reversed(tour[j:k])) + tour[k:])
    "*** --------------  ***"
    return all_possible_neighbours    
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    "*** YOUR CODE HERE ***"
    length = getTourLength(initial_tour)
    tour = initial_tour

    while True:
        neighbours = getNeighbours(tour)
        newLength, newTour = min((getTourLength(x), x) for x in neighbours)
        if newLength < length:
            tourLengthList.append(newLength)
            tour = newTour
            length = newLength
        else:
            minTour = tour
...
def nearestNeighbourTour(initial_city):
    ...
    unexploredList = list(map(lambda x: x + 1, range(cities)))
    unexploredList.remove(initial_city)
    
    while exploredCount != cities:
        _, nextCity = min((getDistance(nodeDict[i], nodeDict[tour[-1]]), i) for i in unexploredList)
        exploredCount += 1
        unexploredList.remove(nextCity)
...
def eucledianTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    edgeList = [[i+1, j+1, getDistance(nodeDict[i+1], nodeDict[j+1])] for i in range(cities) for j in range(i+1, cities)]
    "*** --------------  ***"
...
def finalOrder(mst, initial_city):
    ...
    stack = [initial_city]
    fin_order = [initial_city]

    while stack != []:
        child = getChild(MST, stack[-1])
        if child is not None:
            fin_order.append(child)
            stack.append(child)
        else:
            stack.pop()
    "*** --------------  ***"
...
if __name__ == "__main__":
    ...
    if not args.submit:
        ...
        if args.task == 8:
            tourLengthList = hillClimbWithEucledianMST(args.start_city, generate3optand2optNeighbours)
            with open('task8.csv', 'w') as f:
                f.write(str(tourLengthList))
